  ...
  return (
    <div className="w-full space-y-6">
      ...
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="p-4 rounded-lg bg-white/5 border border-white/10 overflow-x-auto"
      >
        {/* Day labels */}
        <div className="grid grid-cols-7 gap-1 mb-2">
          {displayLabels.map((label) => (
            <div key={label} className="aspect-square flex items-center justify-center text-xs font-semibold text-white/60">
              {label}
            </div>
          ))}
        </div>
            ...
            return (
              <motion.div
                ...
                className={`
                  ...
                  border ${day ? 'border-white/10 hover:border-white/30' : 'border-transparent'}
                `}
      ...
      >
        ...
        <div className="p-3 rounded-lg bg-gradient-to-br from-green-500/20 to-green-600/20 border border-green-500/30">
          <p className="text-xs text-white/60 mb-1">Peak</p>
          <p className="text-lg font-bold text-white">
