  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-slate-900 border border-white/20 rounded-xl shadow-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto"
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-xl font-bold text-white">{eventTitle}</h3>
            <p className="text-sm text-white/60 mt-1">Duration: {currentDuration} day(s)</p>
          </div>
          <motion.button
        ...
        {/* Date Preview */}
        <div className="mb-6">
          <p className="text-xs font-semibold text-white/60 mb-2 uppercase">Preview</p>
        ...
        <div className="flex gap-3">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={onClose}
            className="flex-1 px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 text-white font-medium transition-colors"
          >
            Cancel
          </motion.button>
          <motion.button
