...
export const FinanceQuicklook: React.FC = () => {
  ...
  <h3 className="widget-title flex-1">{t('finance.quicklook')}</h3>
        <div className="hidden md:flex items-center gap-2 text-[10px] opacity-80 flex-wrap max-w-[60%]">
          <span className="px-2 py-1 rounded bg-white/5" title="Days Sales Outstanding">DSO {dso}d</span>
          <span className="px-2 py-1 rounded bg-white/5" title="Gross Margin">GM {grossMargin}%</span>
          <span className="px-2 py-1 rounded bg-white/5" title="Margin % MTD">MTD {marginMtd}%</span>
          <span className="px-2 py-1 rounded bg-white/5" title="Forecast vs Target">FvT {netForecastVsTarget.delta}%</span>
          <span className="px-2 py-1 rounded bg-white/5" title="Margin Δ vs prev">Δ {arrow(marginDelta)} {Math.abs(marginDelta)}%</span>
        </div>
        <div className="flex items-center gap-2 text-[11px]">
          <details className="relative">
            <summary className="cursor-pointer select-none text-[11px] px-2 py-1 rounded-md bg-white/10 hover:bg-white/20">{t('finance.targets')}</summary>
            <div className="absolute right-0 mt-1 z-20 p-3 rounded-md bg-ink-900/95 border border-white/10 shadow-lg min-w-[300px] grid grid-cols-2 gap-2">
              ...
              <input aria-label="incomeMonth" className={`bg-white/10 rounded px-2 py-1 text-[11px] ${targetClass(targets.incomeMonth,'income')}`} type="number" inputMode="numeric" value={targets.incomeMonth} onChange={(e)=>updateTargets({ incomeMonth: Number(e.target.value||0) })} />
              <label className="text-[10px] opacity-70">{t('finance.expenses')}</label>
              <input aria-label="expensesMonth" className={`bg-white/10 rounded px-2 py-1 text-[11px] ${targetClass(targets.expensesMonth,'expenses')}`} type="number" inputMode="numeric" value={targets.expensesMonth} onChange={(e)=>updateTargets({ expensesMonth: Number(e.target.value||0) })} />
              <label className="text-[10px] opacity-70">{t('finance.net')}</label>
              <input aria-label="netMonth" className={`bg-white/10 rounded px-2 py-1 text-[11px] ${targetClass(targets.netMonth,'net')}`} type="number" inputMode="numeric" value={targets.netMonth} onChange={(e)=>updateTargets({ netMonth: Number(e.target.value||0) })} />
              <label className="text-[10px] opacity-80 col-span-2 mt-1">{t('finance.targets.year')}</label>
              <label className="text-[10px] opacity-70">{t('finance.net')}</label>
              <input aria-label="yearNet" className="bg-white/10 rounded px-2 py-1 text-[11px]" type="number" inputMode="numeric" value={targets.yearNet} onChange={(e)=>updateTargets({ yearNet: Number(e.target.value||0) })} />
              <label className="text-[10px] opacity-70">{t('finance.pending')}</label>
              <input aria-label="pending" className={`bg-white/10 rounded px-2 py-1 text-[11px] ${targetClass(targets.pending,'pending')}`} type="number" inputMode="numeric" value={targets.pending} onChange={(e)=>updateTargets({ pending: Number(e.target.value||0) })} />
              <label className="text-[10px] opacity-70">{t('finance.costs')}</label>
              <input aria-label="costsMonth" className={`bg-white/10 rounded px-2 py-1 text-[11px] ${targetClass(targets.costsMonth,'costs')}`} type="number" inputMode="numeric" value={targets.costsMonth} onChange={(e)=>updateTargets({ costsMonth: Number(e.target.value||0) })} />
              <div className="col-span-2 mt-1 text-[10px] opacity-70 flex flex-wrap gap-2">
                <span className="px-1.5 py-0.5 rounded bg-white/5">Fcast: {netForecastVsTarget.forecast}</span>
                <span className="px-1.5 py-0.5 rounded bg-white/5">Δ% {netForecastVsTarget.delta}</span>
              </div>
            </div>
          </details>
          <Link to="/dashboard/finance" onMouseEnter={()=>prefetchByPath('/dashboard/finance')} aria-label={t('finance.openFull') || (t('common.open') + ' ' + t('nav.finance'))} className="text-[11px] px-2 py-1 rounded-md bg-white/10 hover:bg-white/20">{t('finance.openFull') || t('common.open')}</Link>
        </div>
      </header>
