...
export const NetTimeline: React.FC = () => {
  ...
  const { snapshot, netSeries, compareMonthlySeries } = useFinance();
  ...
  const comp = useMemo(()=>{
    ...
    return compareMonthlySeries.months.map((m, idx) => ({ month: m, net: compareMonthlySeries.net[idx] }));
            ...
            {(()=>{
              ...
              if (comparePrev && viewComp) {
                const points2 = viewComp.map((v, idx)=>{
                  const x = Math.max(0, idx*step + step/2);
                  const y = Math.max(0, h - (v.net/Math.max(1,max))*h);
                  return `${x},${y}`;
                }).join(' ');
