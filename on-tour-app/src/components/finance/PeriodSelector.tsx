...
const PeriodSelector: React.FC = () => {
  ...
  return (
    <div className="flex items-center gap-2 text-[11px]">
      <div className="inline-flex bg-white/5 rounded-md overflow-hidden border border-white/10">
      ...
      <label className="inline-flex items-center gap-1 px-2 py-1 rounded bg-white/5 border border-white/10">
      ...
      {preset==='CUSTOM' && (
        <div className="flex items-center gap-1">
          <input type="date" className="bg-white/5 rounded px-2 py-1" value={from} onChange={e=> setFrom(e.target.value)} />
          <span>–</span>
          <input type="date" className="bg-white/5 rounded px-2 py-1" value={to} onChange={e=> setTo(e.target.value)} />
          <button className="px-2 py-1 rounded bg-white/10 hover:bg-white/15" onClick={applyCustom}>{t('common.apply')}</button>
        </div>
      )}
    </div>
  );
