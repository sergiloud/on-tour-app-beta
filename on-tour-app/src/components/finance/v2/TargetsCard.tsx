  ...
  return (
    <div className="p-3 rounded bg-white/5 border border-white/10 text-[12px]">
      ...
      <label className="block mb-2">
        ...
        <input type="number" min={0} value={targets.revenue} onChange={e=> setTargets(s=>({ ...s, revenue: Number(e.target.value) }))} onBlur={onBlur} className="w-full px-2 py-1 rounded bg-white/5 border border-white/10" />
      </label>
      <label className="block">
        <span className="block opacity-80 mb-1">{t('finance.targets.net')||'Net target'}</span>
        <input type="number" min={0} value={targets.net} onChange={e=> setTargets(s=>({ ...s, net: Number(e.target.value) }))} onBlur={onBlur} className="w-full px-2 py-1 rounded bg-white/5 border border-white/10" />
      </label>
