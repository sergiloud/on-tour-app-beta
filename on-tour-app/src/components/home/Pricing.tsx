    ...
    >
      {plans.map((plan) => {
        ...
        return (
          <article
            key={plan.id}
            className={`glass flex h-full flex-col rounded-2xl border border-white/5 bg-white/5 p-6 shadow-sm transition-transform duration-[var(--dur)] ease-[var(--ease)] hover:-translate-y-1 hover:shadow-[var(--elev-2)] ${
          ...
          >
            <header className="flex items-start justify-between gap-4">
              ...
              <div className="text-right">
                <p className="text-3xl font-semibold text-white">{monthlyLabel}</p>
