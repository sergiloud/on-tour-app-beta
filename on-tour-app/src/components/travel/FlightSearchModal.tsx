...
export const FlightSearchModal: React.FC<FlightSearchModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-dark-900 border border-slate-200 dark:border-white/10 rounded-xl max-w-5xl w-full max-h-[90vh] flex flex-col overflow-hidden shadow-xl">
                {/* Header */}
                <div className="flex items-center justify-between p-5 border-b border-white/10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-medium text-slate-900 dark:text-white">{t('travel.search.searchFlights')}</h2>
                            <p className="text-xs text-slate-300 dark:text-white/40">Encuentra las mejores opciones</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                        className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-white/40 hover:text-white transition-colors"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="p-5 border-b border-slate-200 dark:border-white/10 space-y-4">
                    {/* Trip Type */}
                    <div>
                        <label className="text-xs font-medium text-slate-300 dark:text-white/50 uppercase tracking-wide mb-2 block">
                            Tipo de Viaje
                        </label>
                        <div className="flex gap-2">
                            <button
                                ...
                                className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${tripType === 'roundtrip'
                                    ...
                                    : 'bg-dark-800/50 text-slate-400 dark:text-white/60 hover:bg-dark-800 hover:text-slate-600 dark:text-white/80'
                                    }`}
                            >
                                Ida y Vuelta
                            </button>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="relative">
                            <label className="block text-xs font-medium text-slate-400 dark:text-white/60 uppercase tracking-wide mb-2">
                                Origen
                            </label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-3 py-2.5 bg-dark-700 border border-slate-300 dark:border-white/20 rounded-lg text-white text-sm placeholder-slate-400 dark:placeholder-slate-400 dark:placeholder-white/40 focus:bg-dark-600 focus:border-accent-500 focus:outline-none transition-all"
                                />
                            </div>

                            {/* Origin Dropdown */}
                            {showOriginDropdown && originResults.length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-1 bg-dark-700 border-2 border-white/30 rounded-lg shadow-2xl z-[9999] max-h-60 overflow-y-auto">
                                    {originResults.map((airport) => (
                                        <button
                                            key={airport.iata}
                                            onClick={() => selectOrigin(airport)}
                                            className="w-full px-3 py-2.5 hover:bg-accent-500/10 hover:border-l-2 hover:border-accent-500 text-left transition-all border-b border-slate-200 dark:border-white/10 last:border-0"
                                        >
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <div className="text-sm font-semibold text-slate-900 dark:text-white">{airport.city}</div>
                                                    <div className="text-xs text-slate-300 dark:text-white/50">{airport.name}</div>
                                                </div>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-3 py-2.5 bg-dark-700 border border-slate-300 dark:border-white/20 rounded-lg text-white text-sm focus:bg-dark-600 focus:border-accent-500 focus:outline-none transition-all [&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:opacity-70"
                                />
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3 bg-dark-700 border border-slate-300 dark:border-white/20 rounded-lg p-3">
                            ...
                            <span className="text-sm text-slate-300 dark:text-white/50">adultos</span>
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        className="w-full py-3 bg-accent-500 hover:bg-accent-600 disabled:bg-dark-800/50 disabled:text-slate-300 dark:text-white/30 disabled:border disabled:border-slate-200 dark:border-white/10 text-black font-medium rounded-lg transition-all flex items-center justify-center gap-2"
                    >
                        {isSearching ? (
                            <>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {isSearching && (
                        <div className="flex flex-col items-center justify-center py-16">
                            ...
                            <p className="text-xs text-slate-400 dark:text-white/40 mt-1">Esto puede tardar unos segundos</p>
                        </div>
                    )}
                    ...
                    {!isSearching && hasSearched && searchResults.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-16 bg-dark-800/30 rounded-lg border border-white/10">
                            ...
                            <p className="text-sm text-slate-300 dark:text-white/40">Intenta con otras fechas o destinos</p>
                        </div>
                    )}

                    {!isSearching && searchResults.length > 0 && (
                        <div className="space-y-3">
                            <div className="text-sm text-slate-300 dark:text-white/50 mb-3">
                            ...
                            {searchResults.map((flight) => (
                                <div
                                    key={flight.id}
                                    className="bg-dark-700 border border-slate-300 dark:border-white/20 rounded-lg p-4 hover:border-accent-500/50 hover:bg-dark-600 transition-all group"
                                >
                                    {/* Flight Header */}
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="flex items-center gap-3 flex-1">
                                            ...
                                            <div>
                                                <div className="text-sm font-medium text-slate-900 dark:text-white">{flight.airline}</div>
                                                <div className="text-xs text-slate-300 dark:text-white/50 font-mono">{flight.flightNumber}</div>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-2xl font-light text-slate-900 dark:text-white tabular-nums">€{flight.price}</div>
                                            <div className="text-xs text-slate-200 dark:text-white/30">por persona</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4 mb-3">
                                        <div className="flex-1">
                                            ...
                                            <div className="text-xs text-slate-300 dark:text-white/30 font-mono">{flight.origin}</div>
                                        </div>

                                        <div className="flex-1 flex flex-col items-center">
                                            <div className="text-xs text-slate-300 dark:text-white/30 mb-1.5 flex items-center gap-1">
                                                <Clock className="w-3 h-3" />
                                                {flight.duration}
                                            </div>
                                            <div className="relative w-full h-px bg-slate-200 dark:bg-white/20">
                                                ...
                                                <ArrowRight className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 text-slate-200 dark:text-white/20" />
                                            </div>
                                            <div className="text-xs text-slate-300 dark:text-white/30 mt-1.5 bg-dark-900/50 px-2 py-0.5 rounded">
                                        ...
                                        <div className="flex-1 text-right">
                                            ...
                                            <div className="text-xs text-slate-300 dark:text-white/30 font-mono">{flight.destination}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button
                                            ...
                                            className="py-2 bg-dark-800/50 hover:bg-dark-800 border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 text-white rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2"
                                        >
