...
const SmartFlightSearch = React.forwardRef<SmartFlightSearchHandle, SmartFlightSearchProps>(function SmartFlightSearch({ initial, onTripSelected, onResultsChange, onGroupedChange, onDeepLinkChange, onLoadingChange, onErrorChange, onParamsChange }, ref) {
  ...
  const addLeg = () => setSegments(list => {
    const from = list.length ? (list[list.length-1].to||'') : (p.origin||'');
      ...
      <div className="space-y-2">
        ...
        <ShowAutocomplete
          ...
          onSelect={(show)=>{
            ...
            const best = cand && cand.length ? cand[0].iata : '';
  ...
  {/* recent searches */}
        ...
        {state.loading && <div role="status" className="text-xs opacity-70">{t('common.loading')||'Loading'}...</div>}
        <FlightResults results={state.results} onAdd={(r)=> setPendingAdd(r)} />
      </div>
              ...
              {segments.map((s,i)=> (
                <div key={i} className="grid grid-cols-1 sm:grid-cols-4 gap-2 items-end">
                  ...
                  <div className="flex gap-2">
                    <Button type="button" size="sm" variant="ghost" onClick={()=> moveLeg(i, -1)} aria-label={t('travel.multicity.move_up')||'Move up'}>↑</Button>
                    <Button type="button" size="sm" variant="ghost" onClick={()=> moveLeg(i, +1)} aria-label={t('travel.multicity.move_down')||'Move down'}>↓</Button>
...
const ShowAutocomplete: React.FC<{ label: string; onSelect: (s: DemoShow)=> void; }> = ({ label, onSelect }) => {
  const lang = getLang();
  const [shows, setShows] = React.useState<DemoShow[]>([]);
