...
function parseDateToken(tok: string, locale: 'en'|'es'): string | undefined {
...
export function parseTravelQuery(input: string, locale: 'en'|'es' = 'en'): ParsedQuery {
  ...
  if (m) out.adults = Math.max(1, Math.min(8, parseInt(m[1],10)));

  // Bags (singular/plural)
  m = lower.match(/(\d+)\s*(bags?|bultos?|maletas?)/);
  if (m) out.bags = Math.max(0, Math.min(4, parseInt(m[1],10)));
  ...
  if (idxDepart>=0){
    ...
    const iso = parseDateToken(candidate.trim(), locale) || (parts[idxDepart+1] ? parseDateToken(parts[idxDepart+1], locale) : undefined);
    if (iso) out.date = iso;
  }
  ...
  if (idxReturn>=0){
    ...
    const iso = parseDateToken(candidate.trim(), locale) || (parts[idxReturn+1] ? parseDateToken(parts[idxReturn+1], locale) : undefined);
  ...
  if (m){
    const a = findAirportByToken(m[1]); const b = findAirportByToken(m[2]);
