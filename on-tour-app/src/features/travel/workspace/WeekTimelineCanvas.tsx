...
const WeekTimelineCanvas: React.FC<WeekTimelineCanvasProps> = ({ grouped, results, pinnedIds, onAdd, onPin }) => {
  const days = useMemo(() => {
    ...
    if (results.length) {
      const date = results[0].dep.slice(0, 10);
      return [date];
    }
    return [] as string[];
  }, [grouped, results]);
  ...
  const baseByDate: Record<string, FlightResult[]> = useMemo(()=>{
    ...
    if (grouped && Object.keys(grouped).length) {
      for (const k of Object.keys(grouped)) map[k] = grouped[k].slice();
    } else {
  ...
  const shows = useMemo(()=> showStore.getAll().slice().sort((a,b)=> new Date(a.date).getTime()-new Date(b.date).getTime()), []);
                  ...
                  <div className="mt-1 flex items-center justify-between">
                    ...
                    <div className="inline-flex gap-1">
                      <button className="text-[10px] px-1 py-0.5 rounded bg-white/10" onClick={()=>{
                        const idx = days.indexOf(date); if (idx>0) placeOn(r.id, days[idx-1]);
                      }} aria-label={t('travel.move.prev')||'Move to previous day'}>◀︎</button>
                      <button className="text-[10px] px-1 py-0.5 rounded bg-white/10" onClick={()=>{
                        const idx = days.indexOf(date); if (idx<days.length-1) placeOn(r.id, days[idx+1]);
