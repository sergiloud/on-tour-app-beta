    ...
    <form onSubmit={onSave} className="space-y-4">
      <section className="glass rounded p-4 border border-white/10 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="px-3 py-2 rounded bg-white/5 border border-white/12 hover:border-white/20 focus-ring" aria-invalid={!!errors.name} />
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className="px-3 py-2 rounded bg-white/5 border border-white/12 hover:border-white/20 focus-ring" aria-invalid={!!errors.email} />
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input value={form.avatarUrl} onChange={e => setForm(f => ({ ...f, avatarUrl: e.target.value }))} className="px-3 py-2 rounded bg-white/5 border border-white/12 hover:border-white/20 focus-ring" />
          </label>
          <label className="flex flex-col gap-1 text-sm">
            <span className="opacity-80">{t('profile.bio') || 'Bio'}</span>
            <textarea value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} className="px-3 py-2 rounded bg-white/5 border border-white/12 hover:border-white/20 focus-ring" rows={3} />
          </label>
        </div>
      ...
      <section className="glass rounded p-4 border border-white/10 space-y-3">
        ...
        <ul className="text-sm grid grid-cols-1 gap-2">
          {memberships.map(m => (
            <li key={m.org.id} className="flex items-center justify-between gap-2 p-2 rounded bg-white/5 border border-white/10">
              <div>
                <div className="font-medium">{m.org.name}</div>
                <div className="text-xs opacity-70">{m.role}</div>
              </div>
              <button type="button" className="px-2 py-1 text-xs rounded border border-white/20 hover:bg-white/10"
      ...
      <section className="glass rounded p-4 border border-white/10 space-y-3">
        ...
        <div className="flex flex-wrap gap-2 text-sm">
          <button type="button" className="px-3 py-2 rounded border border-white/20 hover:bg-white/10" onClick={() => {
            try {
              const data = {
                profile,
                prefs,
      ...
      <section className="glass rounded p-4 border border-white/10 space-y-3">
        ...
        <div className="space-y-2">
          ...
          <label className="flex items-center justify-between gap-3 text-sm">
            ...
            <select value={mode} onChange={e => setMode(e.target.value as any)} className="bg-white/5 border border-white/12 rounded px-2 py-1">
...
const OrganizationTab: React.FC<{
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex border-b border-white/5 mb-4" role="tablist" aria-label="Organization settings">
...
const ConnectionsTab: React.FC<{
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex border-b border-white/5 mb-4" role="tablist" aria-label="Connections settings">
