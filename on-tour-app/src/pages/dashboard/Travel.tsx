              ...
              <>
                <div className="flex items-center flex-wrap gap-2 text-sm">
                  <label className="inline-flex items-center gap-1">
                    <span className="opacity-70">{t('travel.provider')||'Provider'}</span>
                    <select
                      className="bg-white/5 rounded px-2 py-1 text-xs border border-transparent focus-ring"
                      value={provider}
                      onChange={(e)=> setProvider(e.target.value)}
                    >
                      <option value="mock">{t('provider.mock')||'In-app (mock)'}</option>
                      <option value="google">{t('provider.google')||'Google Flights'}</option>
                    </select>
                  </label>
                  {/* Results sorting control */}
                  <label className="inline-flex items-center gap-1">
                    <span className="opacity-70 hidden sm:inline">{t('travel.results.sortBy') || t('travel.sort.menu') || 'Sort by'}</span>
                    <select
                      className="bg-white/5 rounded px-2 py-1 text-xs border border-transparent focus-ring"
                      value={resultsSort}
                      aria-label={t('travel.results.sortBy') || t('travel.sort.menu') || 'Sort results by'}
                      onChange={(e)=> setResultsSort(e.target.value)}
                    >
                      <option value="price:asc">{t('travel.sort.priceAsc')||'Price (low→high)'}</option>
                      <option value="price:desc">{t('travel.sort.priceDesc')||'Price (high→low)'}</option>
                      <option value="duration:asc">{(t('travel.sort.duration')||'Duration')+' ↑'}</option>
                      <option value="duration:desc">{(t('travel.sort.duration')||'Duration')+' ↓'}</option>
                      <option value="stops:asc">{(t('travel.sort.stops')||'Stops')+' ↑'}</option>
                      <option value="stops:desc">{(t('travel.sort.stops')||'Stops')+' ↓'}</option>
                    </select>
                  </label>
                  <span className="ml-auto opacity-70">
                    {isLoading ? (t('common.loading') || 'Loading…') : `${results.length} ${(t('common.results')||'results')}`}
                  </span>
                </div>
                {error && (
              ...
              return (
                <tr key={r.id} className="glass rounded hover:bg-white/5">
                  ...
                  <td className="px-2 py-1 flex items-center gap-1">
                    ...
                    <button className="px-2 py-0.5 rounded bg-white/5 hover:bg-white/10" onClick={()=> onUnpin(r.id)} aria-label="Unpin">✕</button>
                    {r.deepLink && <a href={r.deepLink} target="_blank" rel="noreferrer" className="px-2 py-0.5 rounded bg-white/5 hover:bg-white/10">↗</a>}
                  </td>
                </tr>
              );
            })}
