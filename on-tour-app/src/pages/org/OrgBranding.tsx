  ...
  if (!org || org.type !== 'artist') {
    return (
      <motion.div className="max-w-[1400px] mx-auto px-3 md:px-4 flex items-center justify-center py-12" layoutId="org-branding">
        <p className="text-xs text-white/70">Branding settings available for artists only</p>
      </motion.div>
    );
  }
    ...
    <motion.div className="max-w-[1400px] mx-auto px-3 md:px-4 space-y-4 pb-8" layoutId="org-branding">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/60 mt-1">Customize your artist profile appearance</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="text-sm font-semibold text-white/90">Logo</h3>
          </div>

          <label className="block">
            <span className="text-xs font-semibold text-white/70 mb-2 block uppercase tracking-wide">Logo URL</span>
            <input
              type="url"
              value={logoUrl}
              onChange={(e) => setLogoUrl(e.target.value)}
              placeholder="https://example.com/logo.png"
              className="w-full px-3 py-2.5 rounded-lg bg-white/5 hover:bg-white/8 border border-white/10 focus:border-accent-500/30 text-sm text-white placeholder-white/40 transition-all focus:outline-none"
            />
            <p className="text-[11px] text-white/60 mt-2">Enter the URL of your logo image</p>
          </label>

          {logoUrl && (
            <div className="mt-4 p-3 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center h-24">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="text-sm font-semibold text-white/90">Primary Color</h3>
          </div>

          <label className="block">
            <span className="text-xs font-semibold text-white/70 mb-3 block uppercase tracking-wide">Brand Color</span>
            <div className="flex items-center gap-3">
              <input
                type="color"
                value={color}
                onChange={(e) => setColor(e.target.value)}
                className="w-16 h-16 p-1 rounded-lg border border-white/10 hover:border-white/20 cursor-pointer transition-colors"
              />
              <div>
                <p className="text-sm font-mono text-white/90">{color}</p>
                <p className="text-[11px] text-white/60 mt-1">Your brand's primary color</p>
              </div>
            </div>
          </label>
          ...
          <div className="mt-4 p-3 rounded-lg bg-white/5 border border-white/10">
            <div className="flex items-center justify-between">
              <span className="text-xs text-white/70 font-semibold">Preview</span>
              <motion.div
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-white/90 mb-1">Pro Tip</h3>
            <p className="text-xs text-white/70">Your branding settings will be visible on your public artist profile and shared with your network.</p>
          </div>
        </div>
      </motion.div>
    </motion.div>
