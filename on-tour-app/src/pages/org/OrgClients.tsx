  ...
  return (
    <div className="px-4 sm:px-6 flex flex-col gap-4 lg:gap-5 pb-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/60 mt-1">Manage your connected artists and agencies</p>
              </div>
            </div>
      ...
      {clientLinks.length === 0 ? (
        <div className="relative overflow-hidden rounded-lg border border-white/10 bg-gradient-to-br from-slate-900/40 to-slate-800/20 backdrop-blur-sm transition-all duration-300 p-12 text-center">
          <Users className="w-12 h-12 mx-auto mb-3 opacity-30" />
          <p className="text-sm text-white/70 mb-1">No connected clients yet</p>
          <p className="text-xs text-white/50 mb-4">Invite artists or connect agencies to get started</p>
          <motion.button
            onClick={() => navigate('/dashboard/org/links')}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          ...
          {clientLinks.map((link, idx) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <span className="px-2 py-1 rounded-full text-[10px] font-medium text-white/70 uppercase tracking-wider">
                  {link.status}
                </span>
              </div>

              <h3 className="text-sm font-semibold text-white mb-1">Artist Link</h3>
              <div className="space-y-1.5 mb-4">
                <div className="flex items-center gap-2">
                  <Lock className="w-3 h-3 text-white/40" />
                  <span className="text-xs text-white/60">
                    Status: <span className="text-white font-medium">{link.status}</span>
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <Lock className="w-3 h-3 text-white/40" />
                  <span className="text-xs text-white/60">
                    Finance: <span className="text-white font-medium">{link.scopes.finance}</span>
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => navigate('/dashboard/org/links')}
                  className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 hover:border-white/20 text-white/70 font-medium text-xs transition-all flex items-center justify-center gap-1"
                >
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-white mb-1">Manage All Connections</h3>
            <p className="text-xs text-white/60">View and edit scopes for all connected clients</p>
          </div>
