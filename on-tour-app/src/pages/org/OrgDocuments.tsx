    ...
    <div className="px-4 sm:px-6 flex flex-col gap-4 lg:gap-5 pb-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-white/60 mt-1">Organize and manage your files</p>
              </div>
            </div>
      ...
      {docs.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm text-slate-500 dark:text-white/70 mb-1">No documents yet</p>
          <p className="text-xs text-slate-300 dark:text-white/50 mb-4">Upload files to organize them here</p>
          <motion.button
            onClick={() => inputRef.current?.click()}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-white/10">
                {docs.map((doc, idx) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm text-slate-500 dark:text-white/70">{formatSize(doc.size)}</span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <motion.button
                          ...
                          className="p-1.5 rounded-lg bg-slate-100 dark:bg-white/5 hover:bg-red-500/10 transition-colors"
                        >
                          <Trash2 className="w-4 h-4 text-slate-400 dark:text-white/60 hover:text-red-400" />
                        </motion.button>
                      </div>
                    </td>
                  </motion.tr>
      ...
      {docs.length > 0 && (
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-white mb-1">Storage Info</p>
