  ...
  return (
    <motion.div className="max-w-[1400px] mx-auto px-3 md:px-4 space-y-4 pb-8" layoutId="org-integrations">
      ...
      <div className="glass rounded-lg border border-white/10 p-3 md:p-4 bg-gradient-to-r from-white/6 to-white/3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/60 mt-1">Connect external services and automate workflows</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h2 className="text-xs font-semibold text-white/70 uppercase tracking-wide px-3 md:px-4">Available Integrations</h2>
          ...
          {integrations.map((integration, idx) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="px-2 py-1 rounded-full bg-white/10 border border-white/15 text-[10px] font-semibold text-white/60 uppercase tracking-wider">
                  {integration.status === 'coming-soon' ? 'Coming Soon' : 'Active'}
                </span>
              </div>
              <h3 className="text-sm font-semibold text-white/90 mb-1">{integration.name}</h3>
              <p className="text-xs text-white/70 mb-3">{integration.description}</p>
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                disabled={integration.status === 'coming-soon'}
                className="w-full px-3 py-1.5 rounded-lg bg-white/8 border border-white/15 hover:border-white/30 hover:bg-white/12 text-white/90 font-semibold text-xs transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-white/90 mb-1">API Access</h3>
            <p className="text-xs text-white/70">Build custom integrations with our API</p>
          </div>
