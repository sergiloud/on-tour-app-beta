...
const IntegrationsModal: React.FC<Props> = ({ open, onClose }) => {
  ...
  return (
    <div role="dialog" aria-modal className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative glass rounded-lg border border-white/10 w-full max-w-md p-4 space-y-3">
        ...
        <input ref={inputRef} type="text" value={calendarKey} onChange={e=>setCalendarKey(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5" placeholder="cal_demo_..." />
