      ...
      <div className="px-8 py-4 border-b border-slate-200 dark:border-white/10 bg-dark-900/50">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <BarChart3 className="w-4 h-4 text-slate-300 dark:text-white/40" />
            <h3 className="text-xs font-medium text-slate-400 dark:text-white/40 uppercase tracking-wider">
              Performance Analysis
            </h3>
          </div>
          <div className="flex items-center gap-4 text-xs">
            <div className="text-slate-300 dark:text-slate-200 dark:text-white/30">
              Total: <span className="tabular-nums text-slate-500 dark:text-white/70 font-medium">{fmtMoney(totalNet)}</span>
            </div>
            <div className="text-slate-300 dark:text-slate-200 dark:text-white/30">
              Items: <span className="tabular-nums text-slate-500 dark:text-white/70">{data.length}</span>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.slice(0, 12).map((row, idx) => {
              ...
              return (
                <button
                  key={row.key}
                  className="bg-dark-900/50 rounded-lg border border-slate-200 dark:border-white/10 p-6 text-left hover:border-accent-500/30 hover:bg-dark-900/70 transition-all group"
                  onClick={() => {
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 min-w-0">
                      <div className="text-xs text-slate-400 dark:text-white/40 mb-1">#{idx + 1}</div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="relative w-16 h-16 flex-shrink-0">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <span className="text-xs font-medium text-slate-700 dark:text-white/90">{margin}%</span>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 space-y-1.5 text-xs min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-slate-300 dark:text-white/50">Revenue</span>
                          <span className="text-slate-500 dark:text-white/70 tabular-nums truncate">{fmtMoney(row.income)}</span>
                        </div>
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-slate-300 dark:text-white/50">Expenses</span>
                          <span className="text-slate-500 dark:text-white/70 tabular-nums truncate">{fmtMoney(row.expenses)}</span>
                        </div>
