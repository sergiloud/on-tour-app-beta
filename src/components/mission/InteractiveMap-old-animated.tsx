...
const InteractiveMapComponent: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  const shows = useMemo(() => {
    ...
    return [...geocodedShows]
      ...
      .slice(0, 100); // Increased limit for better tour visualization
  }, [geocodedShows]);
  
  const { fmtMoney } = useSettings();

  // Smart grouping: Group shows into tour legs
  // A tour leg = consecutive shows within 14 days in the same region or with <7 days gap
  const tourLegs = useMemo((): TourLeg[] => {
    ...
    for (let i = 1; i < shows.length; i++) {
      ...
      // Same country and within 14 days = same leg
      // OR different country but within 7 days = same leg (travel to next country)
