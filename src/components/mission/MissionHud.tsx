...
export const MissionHud: React.FC = () => {
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-slate-200 dark:border-white/10 bg-gradient-to-br from-slate-900/50 to-slate-800/30 backdrop-blur-sm transition-all duration-300 hover:border-slate-300 dark:hover:border-white/20 hover:shadow-lg hover:shadow-purple-500/5">
      ...
      <div className="relative p-5 flex flex-col gap-4" aria-label="Upcoming Agenda">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            to="/dashboard/shows"
            className="flex-1 min-w-[110px] px-3 py-2 rounded-lg bg-slate-200 dark:bg-slate-200 dark:bg-white/10 hover:bg-white/20 border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 transition-all duration-300 flex items-center justify-center gap-2 font-medium text-xs hover:shadow-lg"
          ...
          <button
            className="px-3 py-2 rounded-lg bg-slate-200 dark:bg-slate-200 dark:bg-white/10 hover:bg-white/20 border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 transition-all duration-300 flex items-center justify-center gap-2 font-medium text-xs"
              ...
              <section key={v.id} id={panelId} role="tabpanel" aria-labelledby={tabId} hidden={safeView !== v.id} aria-label={v.label} className="space-y-3 focus:outline-none">
                {v.id === 'upcoming' && (
                  <div className="text-sm space-y-3" aria-label={t('hud.next3weeks')}>
                    {agenda.map(day => (
                      <div key={day.day} className="group border border-slate-200 dark:border-white/10 rounded-lg overflow-hidden hover:border-slate-300 dark:hover:border-white/20 transition-colors">
                              ...
                              <li key={g.city} className="p-3">
                                <div className="flex flex-col gap-2">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="px-2 py-1 rounded bg-slate-200 dark:bg-slate-200 dark:bg-white/10 whitespace-nowrap text-sm font-medium">{g.city}</span>
                                  ...
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <button
                                      className="text-xs px-2.5 py-1 rounded bg-slate-200 dark:bg-slate-200 dark:bg-white/10 hover:bg-white/20 disabled:opacity-50 flex items-center gap-1"
                                      disabled={!canPin}
                                      onClick={() => {
                                        if (!match) return;
                                    ...
                                    <Link
                                      ...
                                      className="text-xs px-2.5 py-1 rounded bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:bg-slate-200 dark:bg-white/10 flex items-center gap-1"
                                    >
