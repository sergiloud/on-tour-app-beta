// Utility to compute net show amount (fee - wht - vat - commissions - costs)
...
export interface NetInputs {
  ...
  vatPct?: number; // percentage 0-100 (VAT/IVA)
...
export function computeNet({ fee=0, whtPct=0, vatPct=0, mgmtPct=0, bookingPct=0, costs=[] }: NetInputs){
  ...
  return fee - wht - vat - commissions - totalCosts;
}
...
export function breakdownNet(inputs: NetInputs){
  ...
  const totalCosts = costs.reduce((s,c)=> s + (c.amount||0), 0);
  const net = fee - wht - vat - commissions - totalCosts;
  return { fee, wht, vat, mgmt, booking, commissions, totalCosts, net };
}