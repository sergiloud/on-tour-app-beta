 ...
 */
import { collection, doc, setDoc, getDoc, getDocs, deleteDoc, query, where, orderBy, onSnapshot, Timestamp } from 'firebase/firestore';
import { db } from '../lib/firebase';
export class FirestoreContactService {
    /**
     * Save contact to Firestore
     */
    static async saveContact(contact, userId) {
        if (!db) {
     ...
     */
    static async getContact(contactId, userId) {
        if (!db) {
            throw new Error('Firestore not initialized');
        }
     ...
     */
    static async getContactsByType(type, userId) {
        if (!db) {
            throw new Error('Firestore not initialized');
        }
        const contactsRef = collection(db, `users/${userId}/contacts`);
        const q = query(contactsRef, where('type', '==', type), orderBy('updatedAt', 'desc'));
        const querySnapshot = await getDocs(q);
        return querySnapshot.docs.map(doc => {
            const data = doc.data();
            return {
     ...
     */
    static async saveContacts(contacts, userId) {
        if (!db) {
            throw new Error('Firestore not initialized');
        }
        const promises = contacts.map(contact => this.saveContact(contact, userId));
        await Promise.all(promises);
    }
    /**
     * Subscribe to real-time updates for user's contacts
     */
    static subscribeToUserContacts(userId, callback) {
        if (!db) {
            throw new Error('Firestore not initialized');
        }
        const contactsRef = collection(db, `users/${userId}/contacts`);
