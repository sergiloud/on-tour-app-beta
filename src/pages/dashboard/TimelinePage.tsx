...
const TimelinePage: React.FC = () => {
  ...
  const getEventIconSvg = (event: TimelineEvent): React.ReactNode => {
    ...
    // Shows module icons
    if (module === 'shows') {
      if (action === 'create' || action.includes('added')) {
        return (
      ...
      if (action === 'update' || action.includes('updated')) {
        return (
          <svg className={iconClass} fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    // Contacts module icons
    if (module === 'contacts' || module === 'crm') {
      return (
        <svg className={iconClass} fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    // Collaboration icons (legacy support)
    ...
    // Settings/audit icons (legacy support)
    if (module === 'audit') {
      return (
        <svg className={iconClass} fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  const getEventColor = (event: TimelineEvent): string => {
    ...
    if (module === 'shows') return 'bg-blue-500/10 text-blue-400 border-blue-500/20';
    if (module === 'finance') return 'bg-green-500/10 text-green-400 border-green-500/20';
    if (module === 'contracts') return 'bg-purple-500/10 text-purple-400 border-purple-500/20';
    if (module === 'venues') return 'bg-orange-500/10 text-orange-400 border-orange-500/20';
    if (module === 'contacts' || module === 'crm') return 'bg-pink-500/10 text-pink-400 border-pink-500/20';
    if (module === 'travel') return 'bg-sky-500/10 text-sky-400 border-sky-500/20';
    if (module === 'collaboration') return 'bg-amber-500/10 text-amber-400 border-amber-500/20';
    if (module === 'audit') return 'bg-slate-500/10 text-slate-400 border-slate-500/20';
    
    return 'bg-slate-500/10 text-slate-400 border-slate-500/20';
  };

  const getImportanceBadge = (importance: string) => {
    if (!importance || importance === 'all') return null;
    ...
    return (
      <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded border ${color}`}>
        {importance}
      </span>
    );
  };
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      ...
                      `}>
                        {/* Icon Badge */}
                        <div className={`relative flex-shrink-0 w-9 h-9 rounded-lg border flex items-center justify-center ${getEventColor(event)}`}>
                          {getEventIconSvg(event)}
                        </div>
